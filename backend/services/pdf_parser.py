...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Save chunks to test/parsed_chunks.json
        out_path = Path(__file__).parent.parent.parent / "test" / "parsed_chunks.json"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(result['chunks'], f, indent=2)
            
        print(f"Saved {len(result['chunks'])} chunks to: {out_path.resolve()}")
        
        # Save images to test/parsed_images.json
        img_out_path = Path(__file__).parent.parent.parent / "test" / "parsed_images.json"
        with open(img_out_path, "w", encoding="utf-8") as f:
            json.dump(result['images'], f, indent=2)
            
        print(f"Saved {len(result['images'])} images to: {img_out_path.resolve()}")
