...
class TopicExtractor:
    ...
    async def _fetch_early_chunks(self, filename: str, limit: int = 30) -> List[Dict[str, Any]]:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            return [{"content": row["content"], "metadata": json.loads(row["metadata"]) if isinstance(row["metadata"], str) else dict(row["metadata"])} for row in rows]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if len(sys.argv) < 2:
            print("Usage: python services/topic_extractor.py <filename_already_in_db.pdf>")
            sys.exit(1)
            
        filename = sys.argv[1]
        print(f"Connecting to DB to extract topics for '{filename}'...")
        
        db_pool = await asyncpg.create_pool(database_url)
        try:
            extractor = TopicExtractor(db_pool)
            topics = await extractor.extract_and_store(filename)
            print("\nFinal Results:")
