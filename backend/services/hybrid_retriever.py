...
if __name__ == "__main__":
    ...
    # Ensure we can import from services if running directly
    ...
    async def main():
        ...
        if len(sys.argv) < 2:
            print('Usage: python -m services.hybrid_retriever "your search query here"')
            sys.exit(1)
            
        query = sys.argv[1]
        print(f"Connecting to DB to search for: '{query}'...")
        
        db_pool = await asyncpg.create_pool(database_url)
        try:
            print("Initializing Embedding Service (loads HuggingFace model)...")
            embedder = EmbeddingService(db_pool)
            
            retriever = HybridRetriever(db_pool, embedder)
            
            print("\nSearching (combining Keyword & Vector search)...")
            results = await retriever.search(query, limit=5)
            
            print(f"\nFound {len(results)} results:\n")
            for i, res in enumerate(results):
                print(f"--- Result {i+1} (Score: {res['combined_score']:.4f}) ---")
                print(f"Source Type: {res['source_type']}")
                print(f"Metadata: {json.dumps(res['metadata'])}")
                print(f"Text Preview: {res['content'][:200].replace(chr(10), ' ')}...")
                print()
